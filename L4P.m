...
%           of X. If Y is a NxM matrix (N points and M replicates), L4P
...
addOptional(p,'st',[],@(x) validateattributes(x,{'numeric'},{'row','real','finite','nonnan','ncols',4}));
addOptional(p,'L',[],@(x) validateattributes(x,{'numeric'},{'row','real','nonnan','ncols',4}));
addOptional(p,'U',[],@(x) validateattributes(x,{'numeric'},{'row','real','nonnan','ncols',4}));
parse(p,x,y,varargin{:});
st=p.Results.st; L=p.Results.L; U=p.Results.U;
clear p
assert(size(x,1)==size(y,1),'X and Y must have the same rows number')
assert(size(x,1)>=4,'Not enough Data points')
...
if isempty(st)
    ...
    st=[min(y) sign(slope) x(Idx) max(y)];
end
