...
% st: allow empty or a 1x4 numeric row vector
addOptional(p, 'st', [], @(v) ...
    (isempty(v) || ...
    (isnumeric(v) && isreal(v) && isrow(v) && numel(v) == 4 && ...
     all(isfinite(v(:))) && ~any(isnan(v(:))))));

% L: allow empty or a 1x4 numeric row vector of bounds
addOptional(p, 'L', [], @(v) ...
    (isempty(v) || ...
    (isnumeric(v) && isreal(v) && isrow(v) && numel(v) == 4 && ...
     all(isfinite(v(:))) && ~any(isnan(v(:))))));

% U: allow empty or a 1x4 numeric row vector of bounds
addOptional(p, 'U', [], @(v) ...
    (isempty(v) || ...
    (isnumeric(v) && isreal(v) && isrow(v) && numel(v) == 4 && ...
     all(isfinite(v(:))) && ~any(isnan(v(:))))));
